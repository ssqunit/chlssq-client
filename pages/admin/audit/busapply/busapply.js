// pages/admin/audit/busapply/busapply.js
...
Page({
  ...
  onDell: function (e) {
    ...
    common.request({
      ...
      data: {
        'shopid': this.data.busJoinInfo.shopid,
        'name': this.data.busJoinInfo.name,
        'owner': this.data.busJoinInfo.owner,
        'ispass': _pass,
        'des': this.data.des
      },
      success: res => {
  ...
  setDellResAndBack: function () {
    ...
    prevPage.setData({
      dirtyObj: { 'shopinfo': this.data.busJoinInfo.shopid }
    });
    wx.navigateBack({
      delta: 1
    })
